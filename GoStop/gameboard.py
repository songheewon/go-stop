...
def main():
    ...
    while True:
        ...
        print("=========================================== information now ===========================================")
        print("ME: " + str(current.player_hand[0]))  # 내 손에 든 카드 출력
        # print("COM: " + str(current.player_hand[1])) # 컴퓨터 손에 든 카드 출력
        print("floor: " + str(current.cards_on_table))  # 바닥 패 출력
        print("top-deck: " + str(current.deck[-1]))
        print("MY SCORE: " + str(current.player_matched[0].calc_score()))  # 내 점수 출력
        print("COM SCORE: " + str(current.player_matched[1].calc_score()))  # 컴퓨터 점수 출력
        print(
            "=======================================================================================================\n")
        can_do = current.step_1()
        if len(can_do) == 0:  # 손에 패가 없는 것임!!!!

            raise Exception("패를 다 냈어요!")
        action = current_player.get_action(current, can_do)  # 현재 Turn의 상황과 낼 수 있는 카드 및 그에 대한 짝 존재 여부를 전달하여 출력
        if type(current_player) == Player.HumanPlayer:
            print('----> 내가 선택한 선택지: {0}\n'.format(str(action)))
        else:
            print('----> 컴퓨터가 선택한 선택지: {0}\n'.format(str(action)))
        last_player = current.current_player  # 기억해두기 (승자 출력을 위함)
        current = current.step_2(action)
        if current.get_result(last_player) == 0:
            if type(players[last_player]) == Player.HumanPlayer:
                print("YOU WIN!")
            else:
                print("YOU LOSE!")
            break
