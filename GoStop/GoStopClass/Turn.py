...
global player_go_cnt, bonus_score
player_go_cnt=[0 for _ in range(2)]  # idx=0 -> player1이 GO를 외친 횟수, idx=1 -> player2가 GO를 외친 횟수
bonus_score = 0  # GO를 외쳐서 더 받는 점수
    ...
    def step_2(self, card_pairs):
        ...
        if card_pairs is not None:
            ...
            if card_pairs.pair is not None:  # 짝 카드가 있다면
                current.cards_on_table.remove(card_pairs.pair)
            else:  # 짝 카드가 없다면
                current.cards_on_table += card_pairs.card  # player가 내려놓은 카드 바닥 패에 추가
        current.recent_pairs = card_pairs
...
class TurnMiddle(Turn):  # 턴 중간 상태 (2단계)
    ...
    def step_2(self, card_pairs):
        current = TurnStart(prev_turn=self)  # 다시 처음으로 돌아간다고 가정 (만약 점수가 났다면 current를 TurnDecision으로 변경해줄 것임)
        if self.recent_pairs.pair is not None:
            current.player_matched[current.current_player] += self.recent_pairs.card
            current.player_matched[current.current_player] += self.recent_pairs.pair
        ...
        if current.player_matched[current.current_player].calc_score() >= 7:  # 현재 턴인 player의 점수가 7점 이상 났다면
            ...
            print(current.player_matched[current.current_player].calc_score())
        else:  # 점수가 나지 않았다면
...
class TurnDecision(Turn):  # 턴 마지막 상태 (3단계)
    ...
    def step_2(self, action):
        if action == GoStop.GO:  # GO를 선택했다면
            player_go_cnt[self.current_player] += 1  # GO를 외친 횟수 카운트
            ...
            if player_go_cnt[current.winner] == 1: # 1번 GO 외쳤으면 1점 추가
                bonus_score += 1
            elif player_go_cnt[current.winner] == 2: # 2번 GO 외쳤으면 2점 추가
                bonus_score += 2
            elif player_go_cnt[current.winner] >= 3: # 3번 이상 GO 외쳤으면 (n-1)배점 추가
                bonus_score += self.player_matched[current.winner].calc_score() * (player_go_cnt[current.winner]-1)+2 # n-1배
        return current
